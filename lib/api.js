...
export async function getArticles() {
  const res = await axios.get(`${baseUrl}/articles?populate[0]=categories`)
  return res.data.data
}

export async function getArticlesByCategory(slug) {
  const res = await axios.get(`${baseUrl}/categories?filters[slug][$eq]=${slug}&populate[0]=articles`)
  return res.data.data
}

export async function getSpecialistsByDiagnose() {
  const res = await axios.get(`${baseUrl}/categories?filters[isDiagnose][$eq]=true&populate[0]=specialists`)
  return res.data.data
}
